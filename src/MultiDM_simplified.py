...
import sys
# from MultiDM_core_ui import UI
...
#         self._geometry = self.mirror.geometry
    ...
    # ----- The zernike calculation has been moved to the zen_calc module.
...
class Mirror():
    def __init__(self, nPixels, nSegments, pattern=None):
        ...
        bds = self.initGeo(self.nPixels) # calculate the borders parameters
    ...
    def whereSegment(self, seg):
        # This is how the 12 x 12 mirror pattern is converted into 256 x 256 or more complex patterns
    ...
    def findSegOffsets(self):
        # The one converts patterns to segnemt
    ...
    def addZernikes(self, zernModes, clear_first = False):
        if clear_first:
            self.clear()
        z = singleZernike(self, zernModes) # OK there's nothing mysterious 
        self.pattern += z
        self.findSegOffsets()
    ...
    def inputMirrorSegs(self, msegs): 
        # Add the 12 x 12 segment values onto the mirror
...
if __name__ == "__main__":
    pixels = 512
    segments = 12
    toSaveDir = "/home/sillycat/Documents/Light_sheet/"
    bds = Mirror(pixels, segments)
    allFiles = []
    print(bds)